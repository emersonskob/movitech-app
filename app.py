    ...
    def salvar_no_azure(df):
        try:
            ...
            for _, row in df.iterrows():
                ...
                st.session_state.usuario  # 👈 AQUI ENTRA O USUÁRIO
                )
...
def app():
    ...
    def salvar_no_azure(df):
        try:
            ...
            for _, row in df.iterrows():
                cursor.execute("""
                    INSERT INTO producao (dia, data, produto, id_produto, lote, etapa)
                    VALUES (?, ?, ?, ?, ?, ?)
                """,
                row["Dia"], row["Data"], row["Produto"],
                row["ID_Produto"], row["Lote"], row["Etapa"])
    ...
    if "dados" not in st.session_state:
        st.session_state.dados = pd.DataFrame(columns=[
            "Dia", "Data", "Produto", "ID_Produto", "Lote", "Etapa"
        ])
    ...
    if st.button("🚀 Gerar Produção Mensal (PPCP)"):
        ...
        st.success("✅ Produção mensal gerada com dias úteis")
...
if st.session_state.logado:
    ...
    tela_login()
>>>>>>> bcaf63ab51042874763a6246c4fb1af4a39d9898
